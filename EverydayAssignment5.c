...
int main() {
    ...
    int frequency[17] = {0}; // Array to store frequency of each highest base
    ...
    for (i = 0; i < numIterations; i++) {
        ...
        frequency[highestBase + 1]++; // Increment the frequency count for the highest base
    }
    
    // Print the frequency of strings for each highest base
    printf("\nthe frequency of strings for each highest base -\n\n");
    for (i = 0; i < 17; i++) {
        if (frequency[i] > 0) {
